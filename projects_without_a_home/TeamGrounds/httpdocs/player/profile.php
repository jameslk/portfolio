...
Libs('controller/tg_interface', 'model/user', 'model/friendship', 'common',
    'controller/sub/comments', 'model/user_profile', 'model/user_game',
    'model/user_gamexp');

/* ----- */

class C_UserProfile extends C_TG_Interface {
    public $title = 'Player Profile';
    public $template_name = 'user/profile.tpl';

    public $user = NULL;
    public $is_self = false;

    public function AddPlayerData($key, $data) {
        $this->template->append('player', array($key => $data), true);
    }

    public function AddPlayerDataArray(array $data) {
        $this->template->append('player', $data, true);
    }
